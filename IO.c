...
static void removeAllSpaces (char *s) {
  ...
  while (s[i]) {
    //while the string is valid
    if (s[i] == remove) {
      //if the string char is a space
      j = i;
      i++;     
      //go to next iteration since the current s[i] element is invalid
      while(s[i]) {
        //while the string is still valid
        if (s[i] != remove) {
          //if the string character is not a bad element
          //swap to create new string
          s[j] = s[i];
          //increment
          j++;
        }
...
static int NotValid(Fraction *f1, Fraction *f2) {
  ...
  //if both fractions are valid and therefore return 1 then the fraction is valid
  if (
    ValidateFraction(f1->numerator, f1->denomenator) && 
    ValidateFraction(f2->numerator, f2->denomenator)) 
    return 0;
  
  //if this is not valid then throw invalid error
  printf("Invalid Fraction. Please Retry. \n");
  return 1;
}
